	...
	<div class="box custom-box">
		...
		<form method="get" action="">
			<p
						...
						<label for="date" style="margin-left:10px;">เลือกวันที่เวลา :</label>

						<label for="date" style="margin-left:10px;">เริ่ม :</label>
						...
						<label for="date" style="margin-left:10px;">สิ่นสุด :</label>
...
วันเวลาประชุม :	<?=  $date_time; ?> 
		...
		<td align="center" style="font-size: 14px; text-align: center; color: black; border-color: inherit;
		<?php
		if ($room_status == 1) {
			echo 'color: white;';
...
<i class="fa fa-edit" style="margin-right: 3px; font-size:15px;"></i>
</a>
...
$('#popupModal').on('show.bs.modal', function (event) {
	...
	function onEdit(booking_id){
		// alert(booking_id);
		window.location = "<?= base_url(); ?>SuperAdmin/room_manage_data/" + booking_id;
