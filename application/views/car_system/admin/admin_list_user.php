...
<label for="status">เลือกรายการ : </label>
...
<select id="status" name="status" required class="narrow-select">
	...
	<?php
	$sql = 'SELECT * FROM user_status_car';
	$query_1 = $this->db->query($sql)->result();

	foreach ($query_1 as $status) {
		$selected = (isset($_GET['status']) && $_GET['status'] == $status->id_car) ? 'selected' : '';
		echo '<option value="' . $status->id_car . '" ' . $selected . '>' . $status->name_car . '</option>';
	}
	?>
</select>
...
<table id="example1" class="table table-bordered table-striped dataTable" role="grid"
	...
	<?php
		...
		foreach ($query as $value) {
			...
			$user_status_car = $value->user_status_car;
			$name_car = $value->name_car;
...
<script>
	...
	function onEdit(id) {
		window.location = "<?= base_url(); ?>car_system/admin/edit_user/" + id;
	}
