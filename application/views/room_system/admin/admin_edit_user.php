...
$user_status_room = @$row->user_status_room;
										...
										<div class="col-sm-7">
											...
											<?php
												...
												foreach ($status_user['status_user'] as $key => $status) {
													$name_address = @$status->name_room;
													$id_address = @$status->id_room;
					...
					<div class="col-md-12 mt-2">
						<div class="button-container">
							...
							<input type="button" value="ย้อนกลับ" class="btn btn-custom rounded-0" style="margin-right: 15px;"
								onclick="window.location='<?= base_url(); ?>room_system/admin/list_user'">
						</div>
...
<script>
	function onSave(id_user) {
		...
		if(edit_cus_name === '' && edit_cus_email === '' && edit_cus_tel === '' && status === '#' && role === '#' ){
...
if(edit_cus_email != ''){
	if (!isValidEmail(edit_cus_email)) {
		...
		if (confirm('คุณต้องการบันทึกข้อมูลใช่หรือไม่')) {
			...
			$.ajax({
				url: "<?= base_url(); ?>room_system/admin/admin_update_user",
				type: 'POST',
				data: pmeters,
				async: false,
				success: function (data) {
					...
					window.location = "<?= base_url(); ?>room_system/admin/list_user";
				}
			});
			return false;
		}
