...
<select id="status" name="status" required class="narrow-select">
	...
	<?php
	$sql = 'SELECT * FROM user_status_room';
	$query_1 = $this->db->query($sql)->result();

	foreach ($query_1 as $status) {
		$selected = (isset($_GET['status']) && $_GET['status'] == $status->id_room) ? 'selected' : '';
		echo '<option value="' . $status->id_room . '" ' . $selected . '>' . $status->name_room . '</option>';
	}
	?>
</select>
...
<table id="example1" class="table table-bordered table-striped dataTable" role="grid"
	...
	<?php
		...
		foreach ($query['query'] as $value) {
			...
			$user_status_room = $value->user_status_room;
...
<script>
	...
	function onEdit(id) {
		window.location = "<?= base_url(); ?>room_system/admin/edit_user/" + id;
	}
