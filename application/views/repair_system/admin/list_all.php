	...
	<section class="content">
		...
		<div class="box custom-box">
			<div class="box-header">
				...
				<a href="<?= base_url(); ?>repair_system/admin/list_all?start_date=<?= isset ($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset ($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>"
					class="btn btn-custom<?= !isset ($_GET['status']) ? ' active' : ''; ?>"
...
&end_date=<?= isset ($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>"
					class="btn btn-warning<?= isset ($_GET['status']) && $_GET['status'] == 1 ? ' active' : ''; ?>"
...
&start_date=<?= isset ($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset ($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>"
					class="btn btn-primary<?= isset ($_GET['status']) && $_GET['status'] == 2 ? ' active' : ''; ?>"
...
&start_date=<?= isset ($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset ($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>"
					class="btn btn-success<?= isset ($_GET['status']) && $_GET['status'] == 3 ? ' active' : ''; ?>"
					style="font-size: 16px;">
					สำเร็จ
					<span class="badge">
...
&start_date=<?= isset ($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset ($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>"
					class="btn btn-customs<?= isset ($_GET['status']) && $_GET['status'] == 4 ? ' active' : ''; ?>"
								...
								foreach ($data['query'] as $key => $rp_case) {
									...
									<tr role="row">
										...
										<td align="center">
											...
											<a href="#" class="<?= ($rp_case_status_num == 5) ? 'disabled-button' : '' ?>"
												<?php if ($rp_case_status_num != 5) { ?> onclick="onEdit(<?= $rp_id ?>)"
												<?php } ?>>
												<i class="fa fa-edit"
													style="color: <?= ($rp_case_status_num == 5) ? 'grey' : 'black' ?>; font-size: 18px;"></i>
											</a>
    ...
    if (confirm("คุณต้องการลบรายการแจ้งซ่อมใช่หรือไม่")) {
        $.ajax({
            ...
            success: function (data) {
                ...
                window.location = "<?= base_url('repair_system/admin/list_all'); ?>";
            }
        });
    }
}
