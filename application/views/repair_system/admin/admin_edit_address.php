...
$sql = "SELECT * FROM rp_address WHERE id_address ='$id_address'";
...
<div class="content-wrapper">
    ...
    <i class="fa fa-pencil"></i>&nbsp; แก้ไขข้อมูลสถานที่
					...
					<div class="col-md-12 mt-2">
						<div class="button-container">
							<input class="btn btn-success" type="submit" name="btnSave" id="btnSave" style="background-color: #4281ff;"
								value="บันทึกข้อมูล" onclick="onSave(<?= $id_address ?>)">

							&nbsp;&nbsp;

							<input type="button" value="ย้อนกลับ" class="btn btn-custom rounded-0"  style="margin-right: 15px;"
								onclick="window.location='<?= base_url(); ?>repair_system/admin/admin_list_address'">
...
<script>
	...
	function onSave(id_address) {
		var edit_name_type = $('#edit_name_type').val();


		if (edit_name_type == '') {
				alert('กรุณากรอกข้อมูลทีต้องการแก้ไข');
		...
		if (confirm('คุณต้องการบันทึกข้อมูลใช่หรือไม่')) {
			if (edit_name_type == '') {
				...
				var pmeters = 'id_address=' + <?= $id_address ?> + '&edit_name_type=' + edit_name_type;
				pmeters = pmeters.replace("undefined", "");

				$.ajax({
					url: "<?= base_url(); ?>repair_system/admin/admin_update_address",
					type: 'POST',
					data: pmeters,
					async: false,
					success: function (data) {
						...
						window.location = "<?= base_url(); ?>repair_system/admin/admin_list_address";
					}
				});
				return false;
