...
<div class="content-wrapper">
	<section class="content">
		<p style="font-size: 20px; background-color: #ffffff; padding: 10px; border-radius: 10px; display: flex; align-items: center;">
			...
			<strong style="flex: 1; display: inline; font-size: 24px;">รายการแจ้งซ่อม</strong>
		</p>
		...
		<form method="get" action="" onsubmit="return validateForm()">
			...
			<p style="font-size: 15px; background-color: #ffffff; padding: 10px; border-radius: 10px; display: flex; align-items: center;">
				...
				<label for="end_date" style="margin-left:10px; font-size: 18px;">ถึง : </label>
			...
			<div class="row">
				...
				<div class="mt-4 col-lg-3 col-6">
					<div class="small-box" style="background-color: #ffb24f; border-radius: 5px;">
						...
						<br>
						<a href="<?= base_url(); ?>repair_system/admin/list_repair?status=1&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?>&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?>&id=<?= $id; ?>" 
				...
				<div class="mt-4 col-lg-3 col-6">
					<div class="small-box" style="background-color: #4281ff; border-radius: 5px;">
						...
						<a href="<?= base_url(); ?>repair_system/admin/list_repair?status=2&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?>&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?>&id=<?= $id; ?>" class="small-box-footer rounded-0">
							ดูเพิ่มเติม <i class="fa fa-arrow-right"></i>
						</a>
				...
				<div class="mt-4 col-lg-3 col-6">
					<div class="small-box" style="background-color: #78d654; border-radius: 5px;">
						...
						<br>
						<a href="<?= base_url(); ?>repair_system/admin/list_repair?status=3&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?>&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?>&id=<?= $id; ?>" class="small-box-footer rounded-0">
							ดูเพิ่มเติม <i class="fa fa-arrow-right"></i>
						</a>
					</div>
				</div>
				...
				<div class="mt-4 col-lg-3 col-6">
					<div class="small-box" style="background-color: #c7c7c7; border-radius: 5px;">
						...
						<br>
						<a href="<?= base_url(); ?>repair_system/admin/list_repair?status=4&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?>&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?>&id=<?= $id; ?>" class="small-box-footer rounded-0">
							ดูเพิ่มเติม <i class="fa fa-arrow-right"></i>
						</a>
			...
			<div class="box-body">
				...
				<div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
					<div class="row">
						...
						<div class="col-sm-7">
							<p style="font-size: 18px; background-color: #f2f2f2; padding: 10px; border-radius: 10px; display: flex; align-items: center;">


								<i class="fa fa-bar-chart" style="margin-right: 10px; background-color: #b3c3ff; padding: 8px; border-radius: 50%; color: white" aria-hidden="true"></i>
						...
						<div class=" col-sm-5">
							...
							<p style="font-size: 18px; background-color: #f2f2f2; padding: 10px; border-radius: 10px; display: flex; align-items: center;">
								...
								<i class="fa fa-list" style="margin-right: 10px; background-color: #b3c3ff; padding: 8px; border-radius: 50%; color: white"></i>
								<strong> งานซ่อมที่ รอดำเนินการ</strong>
							</p>

							<table id="table1" class="table table-bordered table-striped" role="grid" aria-describedby="example1_info">
								<thead>
									<tr role="row" class="info">
										<th tabindex="0" rowspan="1" colspan="1" style="width: 7%;">ลำดับ</th>
										<th tabindex="0" rowspan="1" colspan="1" style="width: 70%;">ประเภทงานซ่อม</th>
								...
								<tbody>
									...
									$selectedStart_date = isset($_GET['start_date']) ? $_GET['start_date'] : date('Y-m-d', strtotime('-3 month'));
									$selectedEnd_date = isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d');
									...
									WHERE rp_case.id_repair = $id AND rp_case.rp_case_status = 1 $whereCondition
