...
<div class="content-wrapper">
	...
	<section class="content">
		<p
			...
			<strong style="flex: 1; display: inline;">รายการแจ้งซ่อมของ
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : date('Y') . '-01-01'; ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-custom<?= !isset($_GET['status']) ? ' active' : ''; ?>" style="background-color: #c79cff; color: white;"> 
    ทั้งหมด
...
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : date('Y') . '-01-01'; ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-primary<?= isset($_GET['status']) && $_GET['status'] == 2 ? ' active' : ''; ?>" style="background-color: #4281ff; color: white;"> 
    กำลังดำเนินงาน
    <span class="badge">
...
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] : date('Y') . '-01-01'; ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-customs<?= isset($_GET['status']) && $_GET['status'] == 4 ? ' active' : ''; ?>" style="background-color: #c7c7c7; color: white;">
    ยกเลิก
    <span class="badge">
					...
					<div class="col-sm-12">
						...
						<table id="example1" class="table table-silver table-bordered table-striped dataTable"
							...
							<tbody>
								...
								&nbsp;	&nbsp;
											<a href="#" class="" onclick="onEdit(<?= $rp_id ?>, <?= $id ?>)">
