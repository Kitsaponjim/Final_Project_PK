	...
	<section class="content">
		<p
			...
			<i class="fa fa-list-alt"
				style="margin-right: 10px; background-color: #b3c3ff;font-size: 18px; padding: 8px; border-radius: 50%; color: white"></i>
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] :date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-custom<?= !isset($_GET['status']) ? ' active' : ''; ?>" style="background-color: #894bdb; color: white;font-size: 16px;"> 
ทั้งหมด
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?status=1
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] :date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-warning<?= isset($_GET['status']) && $_GET['status'] == 1 ? ' active' : ''; ?>" 	style="font-size: 16px;">
รอดำเนินการ
<span class="badge">
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?status=2
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] :date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-primary<?= isset($_GET['status']) && $_GET['status'] == 2 ? ' active' : ''; ?>" style="background-color: #4281ff; color: white;font-size: 16px;"> 
กำลังดำเนินงาน
<span class="badge">
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?status=3
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] :date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-success<?= isset($_GET['status']) && $_GET['status'] == 3 ? ' active' : ''; ?>"	style="font-size: 16px;">
สำเร็จ
<span class="badge">
...
<a href="<?= base_url(); ?>repair_system/employee/list_repair?status=4
&start_date=<?= isset($_GET['start_date']) ? $_GET['start_date'] :date('Y-m-d', strtotime('-3 month')); ?>
&end_date=<?= isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-m-d'); ?>
&id=<?= $id; ?>" class="btn btn-customs<?= isset($_GET['status']) && $_GET['status'] == 4 ? ' active' : ''; ?>" style="background-color: #c7c7c7; color: white;font-size: 16px;">
ยกเลิก
<span class="badge">
...
<br>
								...
								foreach ($data['query'] as $key => $rp_case) {
									...
									<tr role="row">
										...
										<td align="center">
											...
											<a href="#"
												class="<?= ($rp_case_status_num == 3 || $rp_case_status_num == 4 || $rp_case_status_num == 2) ? 'disabled-button' : '' ?>"
												<?php if ($rp_case_status_num != 3 && $rp_case_status_num != 4 && $rp_case_status_num != 2) { ?>
													onclick="onEdit(<?= $rp_id ?>, <?= $id ?>)" <?php } ?>>
												<i class="fa fa-edit"
	...
	<script>
		...
		function onEdit(rp_id, id) {
			window.location = "<?= base_url(); ?>repair_system/employee/manage_data/" + rp_id + "/" + id;
		}
